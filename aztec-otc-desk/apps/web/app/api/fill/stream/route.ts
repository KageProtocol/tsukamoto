...
export async function GET(req: Request) {
  ...
  const scriptPath = path.resolve(root, 'packages/nodejs-demo/scripts/fill_by_id.ts');
  const env = {
    ...process.env,
    ORDER_ID: orderId,
    ...
    API_URL:
      ...
      'http://localhost:3000',
    API_HMAC_SECRET: process.env.OTC_HMAC_SECRET || process.env.API_HMAC_SECRET || '',
  };
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const child = spawn('bun', ['run', scriptPath], { env, cwd: root, stdio: 'pipe' });
