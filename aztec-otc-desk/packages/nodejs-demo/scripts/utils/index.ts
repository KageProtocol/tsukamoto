...
const readAccountsJson = (): any => {
  // Prefer scripts/data/accounts.json; fall back to repo-local accounts.json
  const candidates = [
  ...
  throw new Error(
    "accounts.json not found. Run setup_accounts.ts to generate local keys (kept out of git).",
  );
};
...
export const getAccountFromFs = async (
  ...
  const partialAddress = (await accountManager.getCompleteAddress())
    .partialAddress;
  ...
  return new Promise((resolve) => {
    ...
    const interval = setInterval(async () => {
      ...
      process.stdout.write(
        `Current block: ${currentBlock} (waiting until ${targetBlock})${dots}`,
      );
